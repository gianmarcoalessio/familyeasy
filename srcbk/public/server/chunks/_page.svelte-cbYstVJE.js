...
const Table = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$result.head = previous_head;
    $$rendered = `${``} ${``} <button class="btn btn-primary" data-svelte-h="svelte-11ed6hy">Aggiungi Spesa</button> ${validate_component(Filtraggio, "Filtraggio").$$render($$result, {}, {}, {})} <table class="table table-zebra w-full"><thead data-svelte-h="svelte-1ccmetn"><tr><th>Data</th> <th>Totale</th>  <th class="hidden md:table-cell">Partecipanti</th>  <th class="hidden md:table-cell">Descrizione</th> <th>Categoria</th></tr></thead> <tbody>${each(expenses, (expense) => {
      return `<tr class="hover:bg-neutral-content cursor-pointer"><td>${escape(formatDate(expense.date))}</td> <td>${escape(formatCurrency(expense.totalcost))}</td>  <td class="hidden md:table-cell">${expense.quote.length > 1 ? `<div class="dropdown"><div tabindex="0" role="button" class="btn btn-primary m-1">${escape(formatParticipants(expense.quote))}</div>  <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">${each(expense.quote, (participant) => {
      ...
      })}</ul> </div>` : `${escape(formatParticipants(expense.quote))}`}</td>  <td class="hidden md:table-cell">${escape(expense.description)}</td> <td><div class="badge badge-secondary">${escape(expense.category.name)}</div></td> </tr>`;
    })}</tbody></table> `;
  } while (!$$settled);
  return $$rendered;
});
...
//# sourceMappingURL=_page.svelte-cbYstVJE.js.map
