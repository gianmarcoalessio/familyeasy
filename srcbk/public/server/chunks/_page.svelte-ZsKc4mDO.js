...
const TableBilancio = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${``} <div class="flex flex-row space-x-24 justify-center items-center my-8"><div class="flex flex-col items-center space-y-4 justify-center"><div class="text-5xl" data-svelte-h="svelte-gs36ac">Dare</div> <div class="text-5xl text-error font-bold">${escape(formatCurrency(bilancio.dare))}</div></div> <div class="flex flex-col items-center space-y-4 justify-center"><div class="text-5xl " data-svelte-h="svelte-1fegl1">Avere</div> <div class="text-5xl text-success font-bold">${escape(formatCurrency(bilancio.avere))}</div></div></div> <div class="flex felx-row space-x-2 items-end my-4"><div class="flex-1">${validate_component(Search, "Search").$$render(
      ...
      {},
      {}
    )}</div> ${``}</div> <table class="table table-zebra w-full"> <thead data-svelte-h="svelte-1l7rakn"><tr><th>Data</th> <th>Categoria</th> <th>Descrizione</th> <th>Utente</th> <th>Dare</th> <th>Avere</th></tr></thead> <tbody>${each(expenses, (expense) => {
      return `${each(expense.quote, (quota, i) => {
        return `<tr class="hover:bg-neutral-content cursor-pointer"><td>${i == 0 ? `${escape(formatDate(expense.date))}` : ``}</td> <td>${i == 0 ? `${escape(expense.category.name)}` : ``}</td> <td>${i == 0 ? `${escape(expense.description)}` : ``}</td> <td${add_attribute(
          "class",
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_datilogin = subscribe(datilogin, (value) => $datilogin = value);
  $$unsubscribe_datilogin();
  return `<div class="flex flex-col space-y-4 mx-24 mt-4">${validate_component(Navigator, "Navigator").$$render($$result, {}, {}, {})} <div class="text-6xl font-bold ">Bilancio di @${escape($datilogin.username)}</div></div> <div class="grid grid-cols-1 mx-24 my-4">${validate_component(TableBilancio, "TableBilancio").$$render($$result, {}, {}, {})}</div>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-ZsKc4mDO.js.map
