<script>
    ...
    async function dammiUtenti() {
        try {
            const response = await axios.get(`http://localhost:3000/api/users/search?q=mari`);
            utenti = response.data; // Assuming the user data is directly in the response data
        } catch (err) {
            console.error(err);
            utenti = err.message;
        }
